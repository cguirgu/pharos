...
import { K, font, highlightInk } from '../../src/ui/theme';
  ...
  return (
    <Page>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
        ...
        {hits.length === 0 ? (
          <Caps size={10} ls={1.4} color={K.ink3} style={{ marginTop: 8 }}>
          ...
          hits.map(({ highlight: h }) => (
            <Pressable key={h.id} style={styles.row} onPress={() => router.push(`/highlights/${h.id}`)}>
              <View style={[styles.swatch, { backgroundColor: highlightInk[h.color ?? 'gold'] }]} />
              <View style={styles.entry}>
                <Caps size={8.5} ls={1.4} color={K.ink3}>{h.referenceLabel}</Caps>
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', gap: 12, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: K.ruleDim },
  swatch: { width: 4, alignSelf: 'stretch' },
  entry: { flex: 1 },
  snapshot: { fontFamily: font.body, fontSize: 17, color: K.parch, lineHeight: 24, marginTop: 3 },
  note: { fontFamily: font.bodyItalic, fontSize: 15, color: K.ink2, marginTop: 3 },
});
