/**
 * Onboarding (PRD §5.1): journey + name → starter-rule toggles → notification
 * pre-prompt. On finish, the account's profile is saved and the chosen rule is
 * created. Responsive (iPhone SE → tablet); steps cross-fade.
 */
import React, { useEffect, useRef, useState } from 'react';
import { View, Text, ScrollView, Pressable, StyleSheet, KeyboardAvoidingView, Platform, Animated } from 'react-native';
...
/** A gentle fade-in on mount — keyed by step so each transition cross-fades. */
...
export default function Onboarding() {
  ...
  const finish = async (allowReminders: boolean) => {
    ...
    try {
      await completeOnboarding({ displayName: name, journeyStage: journey, selection: [...selected] });
  ...
  return (
    <Page>
            ...
            <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={scrollPad}>
              ...
              {JOURNEY.map((j) => {
                ...
                return (
                  ...
                  >
                    <Copt size={r.scale(24)} color={on ? t.goldHi : t.ink3} style={{ width: 32, textAlign: 'center' }}>
                      {j.glyph}
                    </Copt>
                    <View style={{ flex: 1 }}>
      ...
      {step === 2 ? (
        <FadeIn key="s2" style={{ flex: 1 }}>
          <ScrollView
            showsVerticalScrollIndicator={false}
            contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 10 }}
          >
